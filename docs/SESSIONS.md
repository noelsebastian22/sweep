...
- Design pass after the fact: removed four `border-left:3px` accent tabs (parked panel, terminal summary, over-budget notice, dashboard active-scan card) and the progress bar's `transition:width`. See "Decided" below.
...
- **No coloured side-tab borders anywhere.** A 3px accent bar down one edge of a card is the single most recognisable tell of a generated UI, and §7's own warning is that this palette drifts generic unless density and mono numerics carry it. Semantics are carried instead by the tinted fill (`warn-bg`), a warn/ok/fail-coloured heading, and the status pill — all existing vocabulary. Elevation stays a hairline.
- **The progress bar does not tween.** It advances on discrete realtime events, the exact count sits beside it, and animating `width` is both banned decorative motion and a layout-animating property. If a future bar needs motion, `transform: scaleX()` is the only acceptable form.
